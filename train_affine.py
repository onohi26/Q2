...
from data.affine_dataset import SynthDataset

from geotnf.transformation import SynthPairTnf

from image.normalization_affine import NormalizeImageDict

from util.train_test_fn_affine import train, validate_model
from util.torch_util import save_checkpoint, str_to_bool

from options.options_affine import ArgumentParser
...
def main():
    ...
    if args.geometric_model=='hom' and args.four_point_hom:
        init_theta = torch.tensor([-1, -1, 1, 1, -1, 1, -1, 1], dtype=torch.float32, device = device)
        model.FeatureRegression.linear.bias.data+=init_theta
    ...
    if args.use_mse_loss:
        ckpt = args.trained_model_fn + '_' + args.geometric_model + '_mse_loss_' + args.feature_extraction_cnn
        checkpoint_path = os.path.join(args.trained_model_dir,
                                       args.trained_model_fn,
                                       ckpt + '.pth.tar')
    else:
        ckpt = args.trained_model_fn + '_' + args.geometric_model + '_grid_loss_' + args.feature_extraction_cnn
        checkpoint_path = os.path.join(args.trained_model_dir,
                                       args.trained_model_fn,
                                       ckpt + '.pth.tar')
