...
class IXIDataset(Dataset):
    ...
    def __init__(self, json_path,dataset_path,source_path,target_path,source_suffix='.raw',output_size=(256,256), transform=None):
        ...
        for dic in json_dict:
            self.img_A_names.append(os.path.join(source_path,dic["filename"]))
            self.img_B_names.append(os.path.join(target_path,dic["filename"]))
    ...
    def __getitem__(self, idx):
        ...
        image_A,label_A,im_size_A, basename_A = self.get_image(self.img_A_names,idx,self.source_suffix)
    ...
    def get_image(self,img_name_list,idx,suffix='.raw'):
        img_name = os.path.join(self.dataset_path, img_name_list[idx])
        if suffix == '.raw':
            with open(img_name, 'rb') as f:
                arr = np.fromfile(f, dtype=np.float64, count=256*256)
                image = arr.reshape((256, 256))
                #gray_image =  (np.maximum(gray_image,0) / gray_image.max()) * 255.0
                image = (image-image.min())/(image.max() - image.min())*255
                
        else:
            image = cv2.imread(os.path.splitext(img_name)[0]+suffix,0)
        ...
        image = torch.Tensor(image.astype(np.float32))
        image = image.unsqueeze(0)
        image_var = Variable(image.unsqueeze(0),requires_grad=False)
