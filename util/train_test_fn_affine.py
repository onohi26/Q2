...
from image.normalization_affine import normalize_image
...
def train(epoch, model, loss_fn, optimizer,
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc='Epoch {}'.format(epoch))):
        ...
        if loss_fn._get_name() == 'MSELoss':
            ...
            source_image = normalize_image(tnf_batch['source_image'], forward=False)
            warped_image_aff = affTnf(source_image, theta.view(-1,2,3))
            target_image = normalize_image(tnf_batch['target_image'], forward=False)
            loss = loss_fn(warped_image_aff, target_image)
        else:
